...
export function computePoints(player, sessions, logs, dailyActivities = [], top14Events = [], taskEvents = [], reactivityEvents = [], bilanEvents = [], challengeEvents = [], convocationEvents = []) {
  ...
  // Présence aux convocations (source = POINTAGE STAFF, cf. team_training_events).
  // Aligné sur le barème séance : présence +10 (+5 ponctualité si présent à
  // l'heure), retard +10 (présent, sans bonus de ponctualité), absence NON
  // annoncée -15. L'absence annoncée ne produit aucun event (neutre).
  (convocationEvents || []).forEach((e) => {
    const inWk = e.date >= wkAgo && e.date <= today;
