  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 800 }}>{t("player.quest.title")}</div>
      </div>
      {list.length === 0 ? (
        <div style={sc({ textAlign: "center", padding: 28, color: "rgba(255,255,255,0.6)", fontSize: 12.5, lineHeight: 1.6 })}>
          {t("player.quest.empty")}<br />{t("player.quest.emptyHint")}
        </div>
      ) : (
        <>
          {todo.length > 0 && <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", letterSpacing: 1, fontWeight: 700, marginBottom: 10 }}>{t("player.quest.toFill")} · {todo.length}</div>}
          {todo.map(Card)}
          {done.length > 0 && <div style={{ fontSize: 10, color: "rgba(255,255,255,0.55)", letterSpacing: 1, fontWeight: 700, margin: "16px 0 10px" }}>{t("player.quest.filledSection")}</div>}
          {done.map(Card)}
        </>
      )}
    </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <button onClick={onClose} style={{ background: "rgba(255,255,255,0.06)", border: `1px solid ${C.border}`, borderRadius: 9, padding: "6px 11px", color: "rgba(255,255,255,0.8)", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{t("common.back")}</button>
      ...
      <button onClick={submit} disabled={preview || busy} style={{ width: "100%", background: preview ? "rgba(255,255,255,0.06)" : accent, border: "none", borderRadius: 12, padding: 14, color: preview ? "rgba(255,255,255,0.5)" : "#fff", fontWeight: 800, fontSize: 14, cursor: preview ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 20, opacity: busy ? 0.7 : 1 }}>
        <CheckCircle size={16} /> {preview ? t("common.previewReadonly") : busy ? t("player.quest.sending") : t("player.quest.send")}
      </button>
    </div>
  );
}
  ...
  if (q.type === "yesno") {
    return (
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => !disabled && onChange(true)} style={pill(value === true)}>{t("player.quest.yes")}</button>
        <button onClick={() => !disabled && onChange(false)} style={pill(value === false)}>{t("player.quest.no")}</button>
      </div>
    );
  }
  if (q.type === "choice") {
  ...
  if (q.type === "repeat") {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div key={f.key} style={{ marginBottom: 7 }}>
                ...
                {f.type === "yesno" ? (
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => !disabled && setRow(i, f.key, true)} style={{ ...pill(r[f.key] === true), padding: "5px 12px", fontSize: 12 }}>{t("player.quest.yes")}</button>
                    <button onClick={() => !disabled && setRow(i, f.key, false)} style={{ ...pill(r[f.key] === false), padding: "5px 12px", fontSize: 12 }}>{t("player.quest.no")}</button>
                  </div>
                ) : (
        ...
        {!disabled && <button onClick={add} style={{ background: "rgba(255,255,255,0.06)", border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 12px", color: "rgba(255,255,255,0.7)", fontSize: 12, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, alignSelf: "flex-start" }}><Plus size={13} /> {t("player.quest.add")}</button>}
        {rows.length === 0 && disabled && <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>—</div>}
      </div>
    );
  }
  // text
  return <textarea value={value ?? ""} disabled={disabled} onChange={(e) => onChange(e.target.value)} placeholder={t("player.quest.answerPlaceholder")} style={{ ...inp, height: 60, resize: "none" }} />;
}
