...
/* Lecteur commun à toutes les séances : démarrer / pause / stop, minuteur,
   durée réglable (nombre de cycles pour la respiration), et récompense à la fin
  ...
  return (
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", background: `radial-gradient(120% 70% at 50% -10%, ${accent}2e 0%, ${C.navy} 55%)`, borderRadius: 16, padding: 14 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        <button onClick={onClose} aria-label={t("onboarding.prev")} style={{ background: "rgba(255,255,255,0.08)", border: `1px solid ${C.border}`, borderRadius: 10, padding: "7px 9px", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center" }}><ChevronLeft size={16} /></button>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15.5, fontWeight: 800, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t(`meditation.sessions.${session.id}.title`)}</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }}>{t(`meditation.sessions.${session.id}.subtitle`)}</div>
        </div>
        {(alreadyDone || finished) && <span title={t("meditation.reward.today")} style={{ fontSize: 9.5, fontWeight: 800, color: C.green, background: `${C.green}22`, border: `1px solid ${C.green}66`, borderRadius: 6, padding: "3px 7px", whiteSpace: "nowrap" }}>{t("meditation.player.doneTag")}</span>}
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "10px 0", minHeight: 320 }}>
        {finished ? (
          <div style={{ textAlign: "center", animation: "medFade .5s ease" }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800 }}>{t("meditation.player.doneTitle")}</div>
            <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.65)", marginTop: 5 }}>{t("meditation.player.doneSub")}</div>
          </div>
        ) : visual()}
      </div>
      ...
      {session.kind === "breathing" && !finished && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginRight: 2 }}>{t("meditation.player.cycles")}</span>
          {CYCLE_PRESETS.map((n) => (
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, paddingBottom: 4 }}>
        {finished ? (
          <button onClick={start} style={btn(accent, { padding: "13px 26px", fontSize: 14 })}>{t("meditation.player.restart")}</button>
        ) : !started ? (
          <button onClick={start} style={btn(accent, { padding: "15px 34px", fontSize: 15.5, boxShadow: `0 6px 20px ${accent}66` })}>{t("meditation.player.start")}</button>
        ) : (
          <>
            <button onClick={stop} style={btn("rgba(255,255,255,0.08)", { padding: "13px 20px", fontSize: 13.5, border: `1px solid ${C.border}` })}>{t("meditation.player.stop")}</button>
            <button onClick={toggle} style={btn(accent, { padding: "14px 30px", fontSize: 15, boxShadow: `0 6px 20px ${accent}55` })}>{running ? t("meditation.player.pause") : t("meditation.player.resume")}</button>
          </>
        )}
      </div>
