...
function PushToggle({ playerId, teamId, accent }) {
  ...
  if (!pushSupported() || state === "unsupported") {
    return (
      <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border2}`, fontSize: 11, color: "rgba(255,255,255,0.5)", lineHeight: 1.5 }}>
        {t("shared.notif.pushUnavailable")}
      </div>
    );
  }
  ...
  const act = async (fn) => {
    ...
    catch (e) { setErr(e?.message || t("shared.notif.fail")); }
    setBusy(false);
  };
  ...
  return (
    <div style={{ padding: "10px 16px", borderBottom: `1px solid ${C.border2}`, display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12.5, fontWeight: 700 }}>{t("shared.notif.pushTitle")}</div>
        <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.55)", marginTop: 1 }}>
          {state === "denied" ? t("shared.notif.pushDenied")
            : subscribed ? t("shared.notif.pushOn")
            : t("shared.notif.pushOff")}
          {err && <span style={{ color: C.coral }}> · {err}</span>}
        </div>
      </div>
      {state !== "denied" && (
        subscribed ? (
          <button onClick={() => act(disablePush)} disabled={busy} style={{ background: "rgba(255,255,255,0.06)", border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 11px", color: "rgba(255,255,255,0.8)", fontSize: 11, fontWeight: 700, cursor: "pointer", flexShrink: 0, opacity: busy ? 0.6 : 1 }}>{t("shared.notif.disable")}</button>
        ) : (
          <button onClick={() => act(() => enablePush(playerId, teamId))} disabled={busy || !playerId} style={{ background: accent, border: "none", borderRadius: 8, padding: "7px 12px", color: "#fff", fontSize: 11.5, fontWeight: 800, cursor: "pointer", flexShrink: 0, opacity: busy || !playerId ? 0.6 : 1 }}>{busy ? "…" : t("shared.notif.enable")}</button>
        )
      )}
    </div>
  );
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 400, display: "flex", justifyContent: "flex-end" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 420, height: "100%", background: C.navy, borderLeft: `1px solid ${C.border2}`, display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", borderBottom: `1px solid ${C.border2}` }}>
          ...
          <div style={{ flex: 1, fontSize: 15, fontWeight: 800 }}>{t("shared.notif.title")}{unread > 0 ? ` · ${unread}` : ""}</div>
          {unread > 0 && <button onClick={markAllRead} style={{ background: "rgba(255,255,255,0.06)", border: `1px solid ${C.border}`, borderRadius: 8, padding: "5px 10px", color: "rgba(255,255,255,0.8)", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{t("shared.notif.allRead")}</button>}
            ...
            return (
              <div key={n.id} onClick={() => open(n)} style={{ display: "flex", gap: 11, padding: "12px 16px", borderBottom: `1px solid ${C.border2}`, cursor: "pointer", background: n.read ? "transparent" : `${accent}12` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.45)", marginTop: 3 }}>{fmtWhen(n.createdAt, t)}</div>
                </div>
              </div>
            );
          })}
