...
export default function TestsEvolution({ player, canEdit = false, accent = C.viol }) {
  ...
  const doCreate = async () => {
    if (!newCamp.name.trim()) return setNote(t("shared.tests.errName"));
    setBusy(true); setNote("");
    try {
      const c = await createCampaign(teamId, newCamp);
      setSelCamp(c.id); setCreating(false); setNewCamp({ name: "", date: todayISO() });
    } catch (e) { setNote(t("shared.tests.errSave", { err: e.message || t("shared.tests.errSaveRetry") })); }
    setBusy(false);
  };

  const doSave = async () => {
    if (!selCamp) return setNote(t("shared.tests.errNoCampaign"));
    setBusy(true); setNote("");
    const metrics = {};
    ...
    } catch (e) { setNote(t("shared.tests.saveFail", { err: e.message || "" })); }
    setBusy(false);
  };
  ...
  const doDeleteCamp = async () => {
    ...
    catch (e) { setNote(t("shared.tests.deleteFail", { err: e.message || "" })); }
    setBusy(false);
  };
  ...
  const editorBtn = canEdit && !editing ? (
    <button onClick={() => setEditing(true)} style={{ background: "none", border: "none", color: accent, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>{t("shared.tests.enter")}</button>
  ) : null;

  return (
    <Section title={t("shared.tests.title")} right={editorBtn}>
      ...
      {editing && (
        <div style={sc({ marginBottom: 12, background: "rgba(255,255,255,0.03)" })}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10 }}>
            {creating ? (
              <>
                <input value={newCamp.name} onChange={(e) => setNewCamp((p) => ({ ...p, name: e.target.value }))} placeholder={t("shared.tests.namePlaceholder")} style={selSt} />
                <input type="date" value={newCamp.date} onChange={(e) => setNewCamp((p) => ({ ...p, date: e.target.value }))} style={{ ...selSt, flex: "0 0 140px" }} />
                <button onClick={doCreate} disabled={busy} style={{ background: accent, border: "none", borderRadius: 8, padding: "8px 10px", color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{t("shared.tests.create")}</button>
              ...
              <>
                <select value={selCamp ?? ""} onChange={(e) => setSelCamp(e.target.value)} style={selSt}>
                  {campaigns.length === 0 && <option value="">{t("shared.tests.noCampaignOption")}</option>}
                  {campaigns.map((c) => <option key={c.id} value={c.id}>{c.name} · {fmtShort(c.date)}</option>)}
                </select>
                <button onClick={() => setCreating(true)} title={t("shared.tests.newCampTitle")} style={{ background: `${accent}22`, border: `1px solid ${accent}66`, borderRadius: 8, padding: "8px 10px", color: accent, fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}><Plus size={13} /> {t("shared.tests.newCampBtn")}</button>
              </>
            )}
          </div>
          ...
          {!creating && selCamp && (
            <>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                <button onClick={() => setEditing(false)} style={{ flex: 1, background: "rgba(255,255,255,0.08)", border: "none", borderRadius: 8, padding: 10, color: "rgba(255,255,255,0.7)", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{t("shared.tests.close")}</button>
                <button onClick={doDeleteCamp} disabled={busy} title={t("shared.tests.deleteCampTitle")} style={{ flex: "0 0 auto", background: "rgba(232,85,59,0.12)", border: `1px solid ${C.coral}44`, borderRadius: 8, padding: "10px 12px", color: C.coral, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{t("shared.tests.deleteCamp")}</button>
                <button onClick={doSave} disabled={busy} style={{ flex: 2, background: C.green, border: "none", borderRadius: 8, padding: 10, color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, opacity: busy ? 0.6 : 1 }}><CheckCircle size={13} /> {t("shared.tests.save")}</button>
              </div>
            </>
          )}
        </div>
      ...
      {campaigns.length === 0 ? (
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>
          {canEdit ? t("shared.tests.emptyEdit") : t("shared.tests.emptyView")}
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ fontSize: 9.5, color: "rgba(255,255,255,0.5)", textAlign: "center", paddingTop: 2 }}>{t("shared.tests.campaignsSummary", { count: campaigns.length, last: campaigns[campaigns.length - 1].name })}</div>
        </div>
      )}
    </Section>
  );
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 2 }}>
            ...
            {prev && (
              <span style={{ fontSize: 10, color: "rgba(255,255,255,0.5)" }}>
                {t("shared.tests.prev", { v: fmtV(prev.v) })}
