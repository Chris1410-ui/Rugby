...
import { fetchDemoPlayers } from "../data/demo.js";
...
/* Espace OWNER (Head of Performance) : voit TOUS les clubs. Sélecteur de club
   ...
   bypass RLS `is_owner()`). Peut aussi OUVRIR LA VUE JOUEUR d'un joueur de démo
   du club (pour une démonstration), rendue avec la session owner. */
export default function OwnerApp({ profile, user, signOut }) {
  const [clubs, setClubs] = useState([]);
  const [team, setTeam] = useState(null);
  const [loading, setLoading] = useState(true);
  const [demoPlayers, setDemoPlayers] = useState([]);
  const [preview, setPreview] = useState(null); // id joueur démo en aperçu
  ...
  // Joueurs démo du club sélectionné (pour l'aperçu). Reset de l'aperçu au
  // changement de club.
  useEffect(() => {
    setPreview(null);
    if (!team) { setDemoPlayers([]); return; }
    let active = true;
    const load = () => fetchDemoPlayers(team).then((d) => { if (active) setDemoPlayers(d); }).catch(() => {});
    load();
    const ch = supabase
      .channel(`owner-demo:${team}`)
  ...
  const previewName = demoPlayers.find((p) => p.id === preview)?.name;
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        <header style={{ position: "sticky", top: 0, zIndex: 30, background: `${C.navy}f2`, backdropFilter: "blur(8px)", borderBottom: `1px solid ${C.border2}`, padding: "12px 18px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {demoPlayers.length > 0 && (
            <select value={preview ?? ""} onChange={(e) => setPreview(e.target.value || null)} style={{ ...selectSt, maxWidth: 150 }} aria-label="Ouvrir la vue d'un joueur démo">
              <option value="">👁 Vue démo…</option>
              {demoPlayers.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          )}
        ...
        {loading ? (
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 16px", background: `${C.viol}22`, borderBottom: `1px solid ${C.viol}55` }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: C.viol }}>👁 VUE JOUEUR (DÉMO) · {previewName}</span>
              <div style={{ flex: 1 }} />
              <button onClick={() => setPreview(null)} style={{ background: "rgba(255,255,255,0.08)", border: `1px solid ${C.border}`, borderRadius: 8, padding: "5px 10px", color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>← Retour owner</button>
            </div>
            <PlayerApp key={preview} profile={{ ...profile, role: "joueur", team_id: team, player_id: preview }} />
          </div>
        ) : (
