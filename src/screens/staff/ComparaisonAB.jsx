...
export default function ComparaisonAB({ teamId, players }) {
  ...
  const B = useMemo(() => analyze(roster.find((p) => p.id === bId), campaigns, results), [roster, bId, campaigns, results]);
  ...
  return (
    <section>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        <button onClick={swap} title="Échanger A et B" style={{ background: "rgba(255,255,255,0.08)", border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 12px", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", flexShrink: 0 }}>⇄</button>
...
function PlayerHead({ data, color, a }) {
  return (
    <div style={{ background: `${color}18`, border: `1px solid ${color}55`, borderRadius: 12, padding: "10px 12px" }}>
      ...
      <div style={{ fontSize: 13.5, fontWeight: 800, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{displayName(data.player)}</div>
      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)", marginTop: 2 }}>{catLabel(data.cat)}</div>
      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginTop: 3 }}>
        <span style={{ color: T14_COL, fontWeight: 800 }}>{data.count}/9</span> Top 14 · {data.lastDate ? `test ${fmtShort(data.lastDate)}` : "aucun test"}
      </div>
    </div>
  );
}
...
function Legend() {
  const dot = (c, l) => (<span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 9.5, color: "rgba(255,255,255,0.6)" }}><span style={{ width: 8, height: 8, borderRadius: 4, background: c }} />{l}</span>);
  return <div style={{ display: "flex", gap: 12, marginTop: 8, flexWrap: "wrap" }}>{dot(A_COL, "A")}{dot(B_COL, "B")}{dot(T14_COL, "Top 14")}</div>;
}
