...
const ACTION_L = { create: "Créer", update: "MAJ", error: "Erreur" };
...
export default function ImportPlayers({ teamId, players = [], onClose }) {
  ...
  const onFile = async (file) => {
    ...
    try {
      ...
      if (!raw.length) { setErr("Fichier vide ou illisible."); setBusy(false); return; }
      const pv = buildPreview(raw, players);
      if (!Object.keys(pv.columnMap).length) { setErr("Aucune colonne reconnue. Utilise le modèle."); setBusy(false); return; }
      setPreview(pv);
      setStep("preview");
    } catch (e) { setErr("Lecture impossible : " + (e.message || "format non supporté")); }
    setBusy(false);
  };
  ...
  const doImport = async () => {
    ...
    } catch (e) { setErr("Échec de l'import : " + (e.message || "")); }
    setBusy(false);
  };
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 320, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px 12px" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 920, background: C.navy, borderRadius: 18, padding: 20, maxHeight: "92vh", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <div style={{ flex: 1, fontSize: 16, fontWeight: 800 }}>Importer des joueurs (Excel / CSV)</div>
        ...
        {step === "file" && (
          <div>
            <div style={sc({ padding: 14, marginBottom: 12 })}>
              <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 8 }}>1. Télécharge le modèle (colonnes normalisées)</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button onClick={() => downloadTemplate("xlsx")} style={tplBtn}><Download size={14} /> Modèle .xlsx</button>
                <button onClick={() => downloadTemplate("csv")} style={tplBtn}><Download size={14} /> Modèle .csv</button>
              </div>
              <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.55)", marginTop: 10, lineHeight: 1.6 }}>
                1 ligne = 1 joueur. Totem (ou numéro) = clé. Colonnes de tests optionnelles. Décimales avec virgule ou point. MAS en m/s.
              </div>
            </div>
            <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, border: `1.5px dashed ${C.border}`, borderRadius: 12, padding: "26px 16px", cursor: "pointer", background: "rgba(255,255,255,0.03)" }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 700 }}>{busy ? "Lecture…" : "2. Choisir un fichier .xlsx ou .csv"}</div>
        ...
        {step === "preview" && preview && (
          <>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
              <Tag c={C.green}>{preview.counts.create} à créer</Tag>
              <Tag c={C.blue}>{preview.counts.update} à mettre à jour</Tag>
              {preview.counts.errors > 0 && <Tag c={C.coral}>{preview.counts.errors} en erreur</Tag>}
              {preview.counts.warnings > 0 && <Tag c={C.amb}>{preview.counts.warnings} avertissement(s)</Tag>}
            </div>
            <div style={{ overflow: "auto", flex: 1, border: `1px solid ${C.border}`, borderRadius: 10, marginBottom: 12 }}>
              <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 620, fontSize: 11.5 }}>
                <thead>
                  <tr>
                    {["", "Totem", "N°", "Poste", "Ligne", "Tests", "Remarques"].map((h, i) => (
                ...
                <tbody>
                  {preview.rows.map((r) => (
                    <tr key={r.index} style={{ opacity: r.action === "error" ? 0.65 : 1 }}>
                      <td style={td}><span style={{ fontSize: 9, fontWeight: 800, color: "#fff", background: ACTION_C[r.action], borderRadius: 5, padding: "2px 6px", whiteSpace: "nowrap" }}>{ACTION_L[r.action]}</span></td>
                      <td style={{ ...td, fontWeight: 700 }}>{r.name || "—"}</td>
                      <td style={td}>{r.num ?? "—"}</td>
                      <td style={td}>{r.pos || "—"}</td>
                      <td style={td}>{r.grp === "avants" ? "Avants" : r.grp === "arrieres" ? "Arrières" : "—"}</td>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setStep("file"); setPreview(null); setErr(""); }} style={{ flex: 1, background: "rgba(255,255,255,0.08)", border: "none", borderRadius: 10, padding: 12, color: "rgba(255,255,255,0.75)", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>← Choisir un autre fichier</button>
              <button onClick={doImport} disabled={busy || validCount === 0} style={{ flex: 2, background: validCount ? C.coral : "rgba(255,255,255,0.1)", border: "none", borderRadius: 10, padding: 12, color: "#fff", fontWeight: 800, fontSize: 13, cursor: validCount ? "pointer" : "default", opacity: busy ? 0.6 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
                <CheckCircle size={15} /> {busy ? "Import…" : `Importer ${validCount} joueur${validCount > 1 ? "s" : ""}`}
              </button>
            </div>
          </>
        )}
        ...
        {step === "done" && summary && (
          <div style={{ textAlign: "center", padding: "20px 10px" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 8 }}>Import terminé</div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", lineHeight: 1.7 }}>
              {summary.created} joueur(s) créé(s) · {summary.updated} mis à jour<br />
              {summary.results > 0 ? `${summary.results} série(s) de tests enregistrée(s) — campagne « Import du ${fmtShort(todayISO())} »` : "Aucun résultat de test importé"}
            </div>
            <button onClick={onClose} style={{ marginTop: 16, background: C.green, border: "none", borderRadius: 10, padding: "11px 20px", color: "#fff", fontWeight: 800, fontSize: 13, cursor: "pointer" }}>Fermer</button>
          </div>
        )}
      </div>
    </div>
