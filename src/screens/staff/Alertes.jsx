  ...
  return (
    <section>
      ...
      <div style={{ borderRadius: 16, background: `linear-gradient(150deg,${accent}33,${C.panel})`, border: `1px solid ${C.border}`, padding: 16, marginBottom: 12 }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1, color: accent, marginBottom: 4 }}>{t("staff.alerts.weekRecap")}</div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.85)", lineHeight: 1.55, marginBottom: 12 }}>
          {t("staff.alerts.summary", { compliance, players: t("staff.alerts.overloadPlayers", { count: overload }), filled: wbFilled, total: players.length })}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
          <KPI label={t("staff.alerts.kpiCompliance")} value={compliance + "%"} color={compliance > 80 ? C.green : compliance > 60 ? C.amb : C.coral} />
          <KPI label={t("staff.alerts.kpiAlerts")} value={alerts.length} color={alerts.length ? C.coral : C.green} />
          <KPI label={t("staff.alerts.kpiOverload")} value={overload} color={overload ? C.coral : C.green} />
        </div>
      </div>

      {/* alertes */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        <div style={{ fontSize: 13, fontWeight: 800, flex: 1 }}>{t("staff.alerts.autoAlerts", { count: alerts.length })}</div>
      </div>
      {cats.length > 1 && (
      ...
      {shown.length === 0 ? (
        <div style={sc({ textAlign: "center", padding: 22, color: "rgba(255,255,255,0.6)", fontSize: 12, marginBottom: 12 })}>{t("staff.alerts.empty")}</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 14 }}>
          {shown.map((a, i) => {
            ...
            <div key={i} onClick={() => setReport({ player: byId(a.pid), reason: { ...a, color: SEVC[a.sev] } })} style={sc({ display: "flex", alignItems: "center", gap: 11, padding: "10px 12px", borderLeft: `3px solid ${SEVC[a.sev]}`, cursor: "pointer" })}>
              ...
              {!readOnly && (
                <button onClick={(e) => { e.stopPropagation(); setThread(byId(a.pid)); }} title={t("staff.alerts.messageTitle")} style={{ background: "rgba(255,255,255,0.06)", border: `1px solid ${C.border}`, borderRadius: 8, padding: 7, color: "rgba(255,255,255,0.6)", cursor: "pointer", display: "flex" }}>
                  <MessageSquare size={15} />
                </button>
              )}
              {!readOnly && (
                <button onClick={(e) => { e.stopPropagation(); markTreated(teamId, a).catch(() => {}); }} title={t("staff.alerts.treatTitle")} style={{ background: `${C.green}1a`, border: `1px solid ${C.green}55`, borderRadius: 8, padding: 7, color: C.green, cursor: "pointer", display: "flex" }}>
                  <CheckCircle size={15} />
                </button>
              )}
            </div>
      ...
      {history.length > 0 && (
        <>
          <button onClick={() => setHistOpen((o) => !o)} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: `1px solid ${C.border}`, borderRadius: 10, padding: "8px 12px", color: "rgba(255,255,255,0.7)", fontSize: 11, fontWeight: 700, cursor: "pointer", marginBottom: 12, textAlign: "left" }}>
            {histOpen ? "▾" : "▸"} {t("staff.alerts.historyToggle", { count: history.length })}
          </button>
          {histOpen && (
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
              {history.map((s) => (
                <div key={s.id} style={sc({ display: "flex", alignItems: "center", gap: 10, padding: "8px 11px", opacity: 0.9 })}>
                  ...
                  {s.treatedAt && !readOnly && <button onClick={() => reopenAlert(s.id).catch(() => {})} title={t("staff.alerts.reopenTitle")} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.5)", padding: 4, fontSize: 13 }}>↩</button>}
                </div>
              ))}
            </div>
          )}
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 330, display: "flex", alignItems: "center", padding: "16px 12px", justifyContent: "center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 560, background: C.panel, borderRadius: 18, padding: 20, maxHeight: "80vh", overflowY: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, marginLeft: 8, fontSize: 15, fontWeight: 800 }}>{t("staff.alerts.recoTitleModal", { name: displayName(player) })}</div>
          <CloseX onClose={onClose} />
        </div>
        {state.loading ? (
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", padding: "20px 0", textAlign: "center" }}>{t("staff.alerts.analyzing")}</div>
        ) : state.error ? (
          <div style={{ fontSize: 12, color: C.coral }}>{t("staff.alerts.error", { err: state.error })}</div>
        ) : (
          <>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.9)", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{state.text}</div>
            <div style={{ marginTop: 14 }}>
              <Tag c={state.source === "claude" ? C.viol : C.gray}>{state.source === "claude" ? t("staff.alerts.sourceClaude") : t("staff.alerts.sourceFallback")}</Tag>
            </div>
          </>
        )}
      </div>
