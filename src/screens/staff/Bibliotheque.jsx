    ...
    <section>
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t("staff.library.search")} style={{ width: "100%", background: "rgba(255,255,255,0.07)", border: `1px solid ${C.border}`, borderRadius: 9, padding: "10px 12px 10px 34px", color: "#fff", fontSize: 13, outline: "none" }} />
        </div>
        {!readOnly && (
          <button onClick={() => setAdding((a) => !a)} style={{ background: `${accent}22`, border: `1px solid ${accent}66`, borderRadius: 9, padding: "0 14px", color: accent, fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            <Plus size={14} /> {t("staff.library.create")}
          </button>
        )}
      </div>
      ...
      {!readOnly && adding && (
        <div style={sc({ marginBottom: 12 })}>
          <input value={nName} onChange={(e) => setNName(e.target.value)} placeholder={t("staff.library.namePlaceholder")} autoFocus style={inp} />
          <select value={nCat} onChange={(e) => setNCat(e.target.value)} style={{ ...inp, fontWeight: 600 }}>
            {EXCATS.map((c) => <option key={c}>{c}</option>)}
          </select>
          <textarea value={nCues} onChange={(e) => setNCues(e.target.value)} placeholder={t("staff.library.cuesPlaceholder")} style={{ ...inp, resize: "none", height: 50 }} />
          <button onClick={addCustom} disabled={!nName.trim() || busy} style={{ width: "100%", background: nName.trim() ? accent : "rgba(255,255,255,0.1)", border: "none", borderRadius: 8, padding: 10, color: "#fff", fontWeight: 700, fontSize: 12, cursor: "pointer", opacity: nName.trim() ? 1 : 0.5 }}>
            {busy ? t("staff.library.adding") : t("staff.library.addToLibrary")}
          </button>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {list.map((e) => (
          <div key={e.id} onClick={() => setSel(e)} style={sc({ padding: 12, cursor: "pointer", borderLeft: `3px solid ${EXCATC[e.cat] || accent}` })}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3 }}>{e.name}{e.custom && <span style={{ fontSize: 8, color: accent, marginLeft: 5 }}>● {t("staff.library.customBadge")}</span>}</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.6)" }}>{e.q}</div>
          </div>
        ))}
      </div>
      {list.length === 0 && <div style={{ textAlign: "center", padding: "34px 18px", color: "rgba(255,255,255,0.6)", fontSize: 12 }}>{t("staff.library.empty")}</div>}
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 300, display: "flex", alignItems: "center", padding: "16px 12px" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 760, margin: "0 auto", background: C.panel, borderRadius: 18, padding: 22, maxHeight: "70vh", overflowY: "auto" }}>
        ...
        <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.6)", letterSpacing: 1, marginBottom: 6 }}>{t("staff.library.cuesTitle")}</div>
